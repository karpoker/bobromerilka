...
async def start(message: types.Message):
	...
	await message.answer("Привет! я бобромерка — бот для чатов (групп)\n\nСмысл бота: бот работает только в чатах. Раз в 24 часа игрок может прописать команду /xwost, где в ответ получит от бота рандомное число.\nРандом работает от -5 см до +10 см.\n\nЕсли у тебя есть вопросы — пиши команду: /help", reply_markup=addgroup)

@dp.message_handler(commands=['help'])
async def help(message: types.Message):
	await message.answer('Команды бота:\n/xwost — Вырастить/уменьшить хвост\n/top_xwost — Топ 10 хвостов чата\n/global_top - Глобальный топ 10 игроков\n\nКонтакты:\nАдмин — @karpfish1')

@dp.message_handler(commands=['xwost'])
async def dick(message: types.Message):
	...
	if str(da) > userInfo.end_date or userInfo.end_date == '':
		if a < 0:
			...
			await message.answer(f'Твой хвост уменьшился на {b} см\nТеперь он равен {userInfo.dick-int(b)} см')
		else:
